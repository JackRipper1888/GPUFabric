...
v1.2 增加目录绑定的 `AssetConfiguration` 固定 Hash、空 Benchmark ID 自动关联、生命周期成色审核事实、市场核验队列和估值三方一致性门禁。流程文件见 [时序图 MMD](asset-assessment-sequence.mmd)、[状态图 MMD](asset-assessment-state.mmd) 和 [状态图 SVG](asset-assessment-state.svg)。
