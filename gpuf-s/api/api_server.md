...
Use `scripts/manage_benchmark_keyring.sh issue` to create a mode-`0600` Ed25519 private key and managed keyring entry. Its `transition` command permits only `active -> retired/revoked` and `retired -> revoked`. For routine rotation, deploy the new active public key first, switch the runner key ID/private key, verify registration, then retire the old key and retain its public metadata until all evidence expires. For suspected compromise, revoke immediately, roll out the keyring, and rerun affected benchmarks. Existing frozen reports remain historical artifacts, but revoked evidence cannot enter a subsequently created report.
